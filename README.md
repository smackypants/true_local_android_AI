...
If you’d like to support development, you can donate via PayPal:

[![Support the project](https://img.shields.io/badge/Support%20the%20project-PayPal-0070BA?logo=paypal&logoColor=white&style=for-the-badge)](https://www.paypal.com/donate/?hosted_button_id=S9KULG56Q58MJ)
...
[![Get it on Google Play](https://play.google.com/intl/en_us/badges/static/images/badges/en_badge_web_generic.png)](https://play.google.com/store/apps/details?id=com.dark.tool_neuron)
...
- **Text generation** — Load any GGUF model (Llama, Mistral, Gemma, Phi,
  Qwen, etc.) and chat with it locally.
- **Image generation** — Stable Diffusion 1.5 on-device, with inpainting
  support.
- **Image tools** — Upscale and segment images locally (depth, style transfer,
  inpainting coming soon).
- **RAG** — Inject PDFs, Word docs, Excel, and EPUB into conversations with
  semantic search.
- **Plugins** — Web search, file manager, calculator, notepad, date/time,
  system info, dev utils — all callable by the LLM.
- **AI memory** — The AI remembers facts about you across conversations, with
  deduplication and a forgetting curve.
- **Text-to-speech** — 10 voices, 5 languages, on-device synthesis.
- **Encrypted storage** — AES-256-GCM with hardware-backed keys for all chat
  data.
...
or [GitHub Releases](https://github.com/Siddhesh2377/ToolNeuron/releases).
...
2. Add a HuggingFace repository (e.g.
...
Select your model, wait for it to load, and start typing. Responses stream in
real-time.
...
> Pick Q4_K_M for a good balance between quality and size. Use Q6_K if your
> device has the RAM for it.
...
- Any GGUF model works — load via file picker (no storage permissions needed,
  uses SAF).
- Configurable parameters: temperature, top-k, top-p, min-p, repeat penalty,
  context length.
...
The RAG pipeline uses hybrid retrieval: FTS4 BM25 + vector search + Reciprocal
Rank Fusion + Maximal Marginal Relevance. Results are injected into the
...
Inspired by [Mem0](https://github.com/mem0ai/mem0). After conversations, the
LLM extracts facts about you and stores them for future context. Deduplication
via Jaccard similarity, with a forgetting curve so stale memories decay. You
can view, edit, and delete memories from the Memory screen.

### Text-to-Speech

On-device TTS via Supertonic (ONNX Runtime). 10 voices (5 female, 5 male), 5
languages (EN, KR, ES, PT, FR). Adjustable speed and quality. Auto-speak option
reads responses aloud.

### Hardware Tuning

Auto-detects CPU topology (P-cores, E-cores) and recommends thread count,
context size, and cache settings. Three modes: Performance, Balanced, Power
Saver.
...
- **Everything stays on-device.** Conversations, generated images, documents,
  TTS audio — none of it leaves your phone.
- **Encrypted storage.** AES-256-GCM with Android KeyStore. On supported
  devices, keys live in the Trusted Execution Environment.
- **No storage permissions.** Models load through Android's file picker (SAF).
  The app can't access arbitrary files.
...
If you hit NDK issues, make sure NDK 26.x is installed via SDK Manager. For
memory issues during build, bump the Gradle heap in `gradle.properties`:
...
See [CONTRIBUTORS.md](CONTRIBUTORS.md) for the project ecosystem and related
repos.
...
2. Email [siddheshsonar2377@gmail.com](mailto:siddheshsonar2377@gmail.com)
...
- [SillyTavern](https://github.com/SillyTavern/SillyTavern) — Character card
  format reference
- [Apache POI](https://poi.apache.org/),
  [PDFBox-Android](https://github.com/TomRoush/PdfBox-Android),
  [EpubLib](https://github.com/psiegman/epublib) — Document parsing
...
[Apache License 2.0](LICENSE) — use it, modify it, distribute it. Attribution
appreciated.
